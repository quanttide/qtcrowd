...
1. **任务列表**：qtcrowd-provider 数据 API published 任务（title / category / business / 状态 / 报酬），未认领显示「可认领」，点击进详情
2. **任务详情**：背景 / 内容 / 输入 / 交付物 / 报酬 / 如何报名（快照缺失的档案段落自动隐藏）
3. **认领**：可认领任务 → 认领按钮 → **调 qtcrowd-provider**（`POST {PROVIDER_URL}/api/tasks/{id}/claim`，
   转发到后台，body `partner_id`，published → accepted）→ 成功写本地认领记录（`data/my-tasks.json`）→ 展示为进行中；
...
|---|---|---|
| `QTCLOUD_CROWD_PROVIDER_URL` | 前台唯一服务端（qtcrowd-provider）根 URL：任务列表读 `{url}/api/tasks`（数据 API，自己桶黄页快照），认领写回 `POST {url}/api/tasks/{id}/claim`（读+写都经它） | 任务目录回退打包 `assets/data/tasks.json`；认领回退 `QTCLOUD_CROWD_BACKEND_API` 直连后台，再未配置 → 本地 mock（`MockClaimApi`，直接成功） |
...
- 任务数据与 site 一致：qtcrowd-provider 数据 API 同源（provider 从后台拉取 published 写自己桶，
  快照为黄页视图 title/description/reward/apply_guide）；未配置 PROVIDER_URL 时回退
  `assets/data/tasks.json`（由 `scripts/sync-tasks.mjs` 从 `../site/src/data/tasks.json` 同步，
  site 的数据源自父仓库 `data/profile`，见 `AGENTS.md`）。
...
```
lib/
├── main.dart                 # 入口接线：qtcrowd-provider 配置 + 仓储集合 + 底部导航
├── models/                   # 领域模型
│   ├── task.dart             # 任务（对齐 site tasks.json 契约；兼容黄页快照 apply_guide/字符串 reward）
...
│   ├── task_repository.dart  # 任务目录（qtcrowd-provider 数据 API Http / 资产 tasks.json 兜底，只读）
...
设计依据：`docs/dev-guide/studio.md`（site 引流，studio 落地）与 `qtcloud-crowd` 的 dev-guide
（前台唯一服务端：qtcrowd-provider 上架 + 数据 API + 转发，读+写都经它）。
