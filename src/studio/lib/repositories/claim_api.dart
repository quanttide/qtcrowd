/// 认领写回 API：前台（studio）认领调后台 API，成功后写本地记录。
///
/// 后台契约（qtcloud-crowd provider 并行改动）：
///   POST {BACKEND_API}/api/tasks/{id}/claim
///   body: {"partner_id": "..."}
///   状态机：published → accepted；非 published 返回 4xx（非法状态，不写本地）。
///
/// 本地开发未配置 QTCLOUD_CROWD_BACKEND_API 时用 [MockClaimApi]（默认成功）。
library;
...
/// HTTP 实现：POST {BACKEND_API}/api/tasks/{id}/claim（body partner_id）。
...
/// 本地 mock：未配置 QTCLOUD_CROWD_BACKEND_API 时的认领实现。
