# =============================================================================
# Site CDN + DNS（crowd.quanttide.com）
#
...
resource "alicloud_cdn_domain_new" "site" {
  domain_name = "crowd.quanttide.com"
  cdn_type    = "web"

  # 源站：OSS 静态网站桶（site 专用桶 qtcrowd-site——上传目标一致）
  sources {
    content  = "qtcrowd-site.oss-cn-hangzhou.aliyuncs.com"
    type     = "oss"
    port     = 80
    priority = 20
  }
  ...
  # terraform 不管理证书内容（避免私钥入库）。crowd.quanttide.com 为单层子域，
